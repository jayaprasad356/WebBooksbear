<?php echo $__env->make("themes.$theme.common.msg", \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
<!-- breadcumb -->
<section class="page_title corner-title overflow-visible">
    <div class="container">
        <div class="row">
            <div class="col-md-12 text-center">
                <h1><?php echo e(__('msg.checkout_summary')); ?></h1>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="<?php echo e(route('home')); ?>"><?php echo e(__('msg.home')); ?></a>
                    </li>
                    <li class="breadcrumb-item active">
                        <?php echo e(__('msg.checkout_summary')); ?>

                    </li>
                </ol>
                <div class="divider-15 d-none d-xl-block"></div>
            </div>
        </div>
    </div>
</section>
<!-- eof breadcumb -->
<div class="main-content">
    <section class="checkout-section ptb-70">
        <div class="container">
            <div class="row">
                <div class="col-lg-3 col-md-12 col-12 mb-4">
                    <div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow rounded account-sidebar account-tab mb-sm-30">
                        <div class="dark-bg tab-title-bg">
                            <div class="heading-part">
                                <div class="sub-title text-center"><span></span><em class="far fa-user"></em> <?php echo e(__('msg.my_account')); ?>

                                </div>
                            </div>
                        </div>
                        <div class="account-tab-inner">
                            <ul class="account-tab-stap">
                                <li>
                                    <a href="<?php echo e(route('cart')); ?>"><em class="fas fa-wallet"></em><?php echo e(__('msg.cart')); ?><em class="fa fa-angle-right"></em> </a>
                                </li>
                                <li>
                                    <a href="<?php echo e(route('checkout-address')); ?>"><em class="fas fa-wallet"></em><?php echo e(__('msg.Address')); ?><em class="fa fa-angle-right"></em> </a>
                                </li>
                                <li class="active">
                                    <a><em class="far fa-heart"></em><?php echo e(__('msg.checkout_summary')); ?><em class="fa fa-angle-right"></em> </a>
                                </li>
                                <li>
                                    <a href="#"><em class="fas fa-digital-tachograph"></em><?php echo e(__('msg.payment')); ?><em class="fa fa-angle-right"></em> </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="col-lg-9 col-md-12 col-12">
                    <div id="data-step1" class="account-content" data-temp="tabdata">
                        <div class="cart-main-content pb-2 pb-lg-5">
                            <div class="container">
                                <div class=" outer_box px-2 py-4 px-md-4 py-md-3 bg-white shadow rounded mb-3">
                                    <div class="section_title d-flex mb-3 align-items-baseline border-bottom">
                                        <h2>
                                            <span class="border-bottom border-primary border-width-2 pb-3 d-inline-block"><?php echo e(__('msg.Order Summary')); ?></span>
                                        </h2>
                                    </div>
                                    <div class="row">
                                        <div class="col-lg-12 col-md-12 col-12">
                                            <div class="table_description">
                                                <div class="cart_page-content">
                                                    <table aria-describedby="cart-table">
                                                        <thead>
                                                            <tr class="cart-header">
                                                                <th scope="col" class="header_product_thumb"><?php echo e(__('msg.Image')); ?></th>
                                                                <th scope="col" class="header_product_name"><?php echo e(__('msg.product')); ?></th>
                                                                <th scope="col" class="header_product-price"><?php echo e(__('msg.price')); ?></th>
                                                                <th scope="col" class="header_product_quantity"><?php echo e(__('msg.qty')); ?></th>
                                                                <th scope="col" class="header_product_total"><?php echo e(__('msg.subtotal')); ?></th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <?php $ready_to_checkout = '1'; ?>
                                                            <?php if(isset($data['data']['data']) && is_array($data['data']['data']) && count($data['data']['data'])): ?>
                                                            <?php $__currentLoopData = $data['data']['data']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                            <?php if(isset($p->item)): ?>
                                                            <tr>
                                                                <td class="header_product_thumb"><a href="#"><img class="lazy" data-original="<?php echo e($p->item[0]->image); ?>" alt=""></a></td>
                                                                <td class="header_product_name">
                                                                    <?php if(($p->item[0]->is_item_deliverable == false) && (Cache::get('pincode_no') == true)): ?>
                                                                    <p class="deliver_notice"><?php echo e(__('msg.This_item_is_not_deliverable_in_selected_address')); ?></p>
                                                                    <?php $ready_to_checkout = '0'; ?>
                                                                    <?php endif; ?>
                                                                    <a href="#"><?php echo e(strtoupper($p->item[0]->name) ?? '-'); ?></a>
                                                                    <p class="small text-muted text-center"><?php echo e(get_varient_name($p->item[0])); ?>

                                                                        <?php if(intval($p->item[0]->discounted_price)): ?>
                                                                        (<?php echo e(intval($p->item[0]->discounted_price)); ?> X <?php echo e(($p->qty ?? 1)); ?>)
                                                                        <?php else: ?>
                                                                        (<?php echo e(intval($p->item[0]->price)); ?> X <?php echo e(($p->qty ?? 1)); ?>)
                                                                        <?php endif; ?>
                                                                        <br><?php echo e(__('msg.tax')." (".$p->item[0]->tax_percentage); ?>% <?php echo e($p->item[0]->tax_title); ?>)
                                                                    </p>
                                                                </td>
                                                                <td class="header_product-price">
                                                                    <?php if(intval($p->item[0]->discounted_price)): ?>
                                                                    <?php if(isset($p->item[0]->tax_percentage) && $p->item[0]->tax_percentage > 0): ?>
                                                                    <?php echo e($p->item[0]->discounted_price+($p->item[0]->discounted_price*$p->item[0]->tax_percentage/100) ?? ''); ?>

                                                                    <?php else: ?>
                                                                    <?php echo e($p->item[0]->discounted_price ?? ''); ?>

                                                                    <?php endif; ?>
                                                                    <?php else: ?>
                                                                    <?php if(isset($p->item[0]->tax_percentage) && $p->item[0]->tax_percentage > 0): ?>
                                                                    <?php echo e($p->item[0]->price+($p->item[0]->price*$p->item[0]->tax_percentage/100) ?? ''); ?>

                                                                    <?php else: ?>
                                                                    <?php echo e($p->item[0]->price ?? ''); ?>

                                                                    <?php endif; ?>
                                                                    <?php endif; ?>
                                                                </td>
                                                                <td class="header_product_quantity"><?php echo e($p->qty); ?></td>
                                                                <td class="header_product_total">
                                                                    <?php if(intval($p->item[0]->discounted_price)): ?>
                                                                    <?php if(isset($p->item[0]->tax_percentage) && $p->item[0]->tax_percentage > 0): ?>
                                                                    <?php echo e(($p->item[0]->discounted_price+($p->item[0]->discounted_price*$p->item[0]->tax_percentage/100)) * ($p->qty ?? 1)); ?>

                                                                    <?php else: ?>
                                                                    <?php echo e($p->item[0]->discounted_price * ($p->qty ?? 1)); ?>

                                                                    <?php endif; ?>
                                                                    <?php else: ?>
                                                                    <?php if(isset($p->item[0]->tax_percentage) && $p->item[0]->tax_percentage > 0): ?>
                                                                    <?php echo e(($p->item[0]->price+($p->item[0]->price*$p->item[0]->tax_percentage/100)) * ($p->qty ?? 1)); ?>

                                                                    <?php else: ?>
                                                                    <?php echo e($p->item[0]->price * ($p->qty ?? 1)); ?>

                                                                    <?php endif; ?>
                                                                    <?php endif; ?>
                                                                </td>
                                                            </tr>
                                                            <?php endif; ?>
                                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                            <?php endif; ?>
                                                        </tbody>
                                                    </table>
                                                </div>
                                                <div class="cart_submit">
                                                    <a href="<?php echo e(route('shop')); ?>" class="btn cart_shopping"><em class="fas fa-angle-double-left"></em>&nbsp;&nbsp;<?php echo e(__('msg.Continue Shopping')); ?></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class=" outer_box px-2 py-4 px-md-4 py-md-3 bg-white shadow rounded">
                                    <div class="row">
                                        <div class="col-lg-4 col-md-6 col-12 ">
                                            <div class="discount-code-wrapper">
                                                <div class="title-wrap">
                                                    <h4 class="cart-bottom-title section-bg-gray"><?php echo e(__('msg.Use Coupon Code')); ?></h4>
                                                </div>
                                                <?php if(intval($data['cart']['coupon'] ?? 0)): ?>
                                                <div class="form-group" id='couponAppliedDiv'>
                                                    <label class="title-sec"><?php echo e(__('msg.coupon_code')); ?></label>
                                                    <div class="alert alert-success"><?php echo e($data['cart']['coupon']['promo_code_message']); ?></div>
                                                    <div class="input-group">
                                                        <input type="text" class="form-control" value="<?php echo e($data['cart']['coupon']['promo_code']); ?>" disabled="disabled" placeholder="Coupon code">
                                                        <span class="input-group-append">
                                                            <a href="<?php echo e(route('coupon-remove')); ?>" class="btn btn-danger" id='removeCoupon'>x</a>
                                                        </span>
                                                    </div>
                                                </div>
                                                <?php endif; ?>
                                                <div class="discount-code">
                                                    <p><?php echo e(__('msg.Enter your coupon code if you have one.')); ?></p>
                                                    <form action="<?php echo e(route('coupon-apply')); ?>" method="POST" class='ajax-form <?php echo e(intval($data['cart']['coupon'] ?? 0) ? 'address-hide' : ''); ?>' id='couponForm'>
                                                        <input type="hidden" name="total" value="<?php echo e($data['data']['total_amount'] ?? ''); ?>">
                                                        <div class='formResponse'></div>
                                                        <div class="input-group">
                                                            <input type="text" class="form-control" name="coupon" value="<?php echo e($data['cart']['coupon']['promo_code'] ?? ''); ?>" placeholder="Coupon code">
                                                            <input type="hidden" class="form-control" name="address_id" value="<?php echo e($data['address']->id ?? ''); ?>">
                                                            <span class="input-group-append">
                                                                <button class="btn btn-primary"><?php echo e(__('msg.APPLY COUPON')); ?></button>
                                                            </span>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                                        <?php if(isset($data['data']['code_list']['data']) && is_array($data['data']['code_list']['data']) && count($data['data']['code_list']['data'])): ?>
                                        <?php if(isset($data['data']['code_list']['data'][0]->id)): ?>
                                        <div class="col-lg-4 col-md-4 col-12 ">
                                            <div class="discount-code-wrapper coupan__wrapper">
                                                <div class="title-wrap">
                                                    <h4 class="cart-bottom-title section-bg-gray"><?php echo e(__('msg.offers')); ?></h4>
                                                </div>

                                                <div class="discount-code">
                                                    <p><?php echo e(__('msg.copy_and_paste_code_in_coupan_code_to_get_exciting_discount')); ?></p>

                                                    
                                                    <?php if(isset($data['data']['code_list']) && is_array($data['data']['code_list']['data']) && count($data['data']['code_list']['data'])): ?>
                                                    <?php $__currentLoopData = $data['data']['code_list']['data']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <?php if(isset($c->id) && !($c->is_validate[0]->error)): ?>
                                                    <div class="inner__sec_discount mb-2">
                                                        <h5 class=""><?php echo e($c->is_validate[0]->promo_code_message); ?></h5>
                                                        <p class="themeclr">You will save <?php echo e($c->is_validate[0]->discount); ?> with this code</p>
                                                        <p class="pt-1 coupan_text"><?php echo e($c->promo_code); ?></p>
                                                    </div>
                                                    <?php else: ?>
                                                    <div class="inner__sec_discount mb-2">
                                                        <h5 class=""><?php echo e($c->message); ?></h5>
                                                        <p class="redclr"><?php echo e($c->is_validate[0]->message); ?></p>
                                                        <p class="pt-1 coupan_text"><?php echo e($c->promo_code); ?></p>
                                                    </div>
                                                    <?php endif; ?>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                    <?php endif; ?>
                                                </div>

                                            </div>
                                        </div>
                                        <?php endif; ?>
                                        <?php endif; ?>

                                        <div class="col-lg-4 col-md-6 col-12 ">
                                            <div class="grand-total-content">
                                                <div class="title-wrap">
                                                    <h4 class="cart-bottom-title section-bg-gary-cart"><?php echo e(__('msg.Cart Total')); ?></h4>
                                                </div>
                                                <h5><?php echo e(__('msg.subtotal')); ?> : <span><?php echo e(get_price(sprintf("%0.2f",$data['cart']['subtotal'] ?? '-'))); ?></span></h5>
                                                <?php if(isset($data['cart']['coupon']['discount'])  && floatval($data['cart']['coupon']['discount'])): ?>
                                                <h5><?php echo e(__('msg.discount')); ?> : <span>-
                                                        <?php echo e(get_price(sprintf("%0.2f",$data['cart']['coupon']['discount'])) ?? '-'); ?></span>
                                                </h5>
                                                <?php endif; ?>
                                                <?php if(isset($data['cart']['saved_price']) && floatval($data['cart']['saved_price'])): ?>
                                                <h5><?php echo e(__('msg.saved_price')); ?> : <span>
                                                        <?php echo e(get_price($data['cart']['saved_price'])); ?></span>
                                                </h5>
                                                <?php endif; ?>
                                                <?php if(isset($data['cart']['shipping']) && floatval($data['cart']['shipping'])): ?>
                                                <h5><?php echo e(__('msg.delivery_charge')); ?> : <span>
                                                        <?php echo e(get_price($data['cart']['shipping'])); ?></span>
                                                </h5>
                                                <?php endif; ?>
                                                <?php if($ready_to_checkout == '1'): ?>
                                                <a href="<?php echo e(route('checkout-payment')); ?>"><?php echo e(__('msg.Proceed to Checkout')); ?>&nbsp;&nbsp;<em class="fas fa-angle-double-right"></em></a>
                                                <?php else: ?>
                                                <a class="checkout-dbutton"><?php echo e(__('msg.checkout')); ?>

                                                    <em class="fas fa-angle-double-right"></em>
                                                </a>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</div><?php /**PATH C:\xampp\htdocs\webbooksbear\resources\views/themes/eCart/checkout_summary.blade.php ENDPATH**/ ?>